    ...
    for (const auto &rls : lifecycle.getLayers()) {
      if (rls && rls->id == walk) {
        printf("    chain[%d]: id=%u name=%s parent=%u canBeRoot=%d\n", d,
               rls->id, rls->name.c_str(), rls->parentId,
               rls->canBeRoot ? 1 : 0);
        parent = rls->parentId;
        found = true;
        break;
...
inline DiffStats diffSnapshotsAgainstSf(
    ...
    if (sf.has_bounds()) {
      ...
      if (!(near(ours->geomLayerBounds.left, b.left()) &&
        ...
        mismatches += android::base::StringPrintf(
            ...
            ours->geomLayerBounds.right, ours->geomLayerBounds.bottom, b.left(),
            b.top(), b.right(), b.bottom());
      }
    }
    if (sf.has_is_opaque() && ours->contentOpaque != sf.is_opaque())
      mismatches += android::base::StringPrintf(" opaque{%d vs %d}",
                                                ours->contentOpaque ? 1 : 0,
                                                sf.is_opaque() ? 1 : 0);
  ...
  printf("  totals: both=%d (%d differ) only_ours=%d only_sf=%d\n", stats.both,
         stats.diffFields, stats.onlyOurs, stats.onlySf);
  return stats;
}
