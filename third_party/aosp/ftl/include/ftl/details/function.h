...
// Converts a member function pointer type `Ret(Class::*)(Args...)` to an
// equivalent non-member function type `Ret(Args...)`.

template <typename> struct remove_member_function_pointer;
...
template <std::size_t N> struct function_opaque_storage {
  // The actual type used for the opaque storage. An `N` of zero specifies the
  // minimum useful size, which allows a lambda with zero or one capture args.
  using type = std::array<std::intptr_t, N + 1>;

  template <typename S>
  static constexpr bool require_trivially_copyable =
      std::is_trivially_copyable_v<S>;

  template <typename S>
  static constexpr bool require_trivially_destructible =
      std::is_trivially_destructible_v<S>;

  template <typename S>
  static constexpr bool require_will_fit_in_opaque_storage =
      sizeof(S) <= sizeof(type);
  ...
  template <typename S> static type opaque_copy(const S &src) {
    // TODO: Replace with C++20 concepts/constraints which can give more
    // details.
    static_assert(require_trivially_copyable<S>,
                  "ftl::Function can only store lambdas that capture trivially "
                  "copyable data.");
    static_assert(require_trivially_destructible<S>,
                  "ftl::Function can only store lambdas that capture trivially "
                  "destructible data.");
    static_assert(require_will_fit_in_opaque_storage<S>,
                  "ftl::Function has limited storage for lambda captured "
                  "state. Maybe you need to "
                  "increase N?");
...
// Traits class to help determine the template parameters to use for a
// ftl::Function, given a function object.

template <typename F, typename = decltype(&F::operator())>
struct function_traits {
  // The function type `F` with which to instantiate the `Function<F, N>`
  // template.
  using type = remove_member_function_pointer_t<&F::operator()>;

  // The (minimum) size `N` with which to instantiate the `Function<F, N>`
  // template.
