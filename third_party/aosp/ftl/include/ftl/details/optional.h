...
template <typename F, typename T> struct transform_result {
...
template <typename F, typename T> struct and_then_result {
  using type = remove_cvref_t<std::invoke_result_t<F, T>>;
  static_assert(is_optional<type>{},
                "and_then function must return an optional");
};

template <typename F, typename T>
using and_then_result_t = typename and_then_result<F, T>::type;

template <typename F, typename T> struct or_else_result {
  using type = remove_cvref_t<std::invoke_result_t<F>>;
  static_assert(std::is_same_v<type, std::optional<T>> ||
                    std::is_same_v<type, Optional<T>>,
