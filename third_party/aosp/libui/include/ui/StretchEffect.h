...
struct StretchEffect : public LightFlattenablePod<StretchEffect> {
  ...
  bool operator==(const StretchEffect &other) const {
    return width == other.width && height == other.height &&
           vectorX == other.vectorX && vectorY == other.vectorY &&
           maxAmountX == other.maxAmountX && maxAmountY == other.maxAmountY &&
  ...
  void sanitize() {
    // If the area is empty, or the max amount is zero, then reset back to
    // defaults
