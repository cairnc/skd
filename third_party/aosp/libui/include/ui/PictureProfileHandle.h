...
/**
 * An opaque value that uniquely identifies a picture profile, or a set of
 * parameters, which describes the configuration of a picture processing
 * pipeline that is applied to a graphic buffer to enhance its quality prior to
 * rendering on the display.
 */
typedef int64_t PictureProfileId;

/**
 * A picture profile handle wraps the picture profile ID for type-safety, and
 * represents an opaque handle that doesn't have the performance drawbacks of
 * Binders.
 */
...
public:
  ...
  inline bool operator==(const PictureProfileHandle &rhs) {
    return mId == rhs.mId;
  }
  inline bool operator!=(const PictureProfileHandle &rhs) {
    return !(*this == rhs);
