...
struct Size {
  ...
  template <typename T> void setWidth(T v) { width = clamp<int32_t>(v); }

  template <typename T> void setHeight(T v) { height = clamp<int32_t>(v); }

  void set(Size size) { *this = size; }

  template <typename T> void set(T w, T h) { set(Size(w, h)); }
  ...
  static constexpr remove_cvref_t<ToType> clamp(FromType v) {
    ...
    using CommonLowestType = std::conditional_t<
        ...
        std::make_signed_t<std::conditional_t<
            CommonLimits::is_integer, CommonHighestType, int /* not used */>>,
        CommonHighestType>;
...
inline void Size::makeInvalid() { set(kInvalidSize); }

inline void Size::clear() { set(kEmptySize); }

inline bool operator==(Size lhs, Size rhs) {
  return lhs.width == rhs.width && lhs.height == rhs.height;
}

inline bool Size::isEmpty() const { return *this == kEmptySize; }

inline bool operator!=(Size lhs, Size rhs) { return !(lhs == rhs); }
