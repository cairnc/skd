...
bool Transform::isZero(float f) { return fabs(f) <= EPSILON; }

bool Transform::absIsOne(float f) { return isZero(fabs(f) - 1.0f); }
...
const vec3 &Transform::operator[](size_t i) const { return mMatrix[i]; }

// x translate
float Transform::tx() const { return mMatrix[2][0]; }

// y translate
float Transform::ty() const { return mMatrix[2][1]; }

float Transform::dsdx() const { return mMatrix[0][0]; }

float Transform::dtdx() const { return mMatrix[1][0]; }

float Transform::dtdy() const { return mMatrix[0][1]; }

float Transform::dsdy() const { return mMatrix[1][1]; }
...
Rect Transform::makeBounds(int w, int h) const { return transform(Rect(w, h)); }
...
Rect Transform::transform(const Rect &bounds, bool roundOutwards) const {
  ...
  if (roundOutwards) {
    r.left =
        static_cast<int32_t>(floorf(std::min({lt[0], rt[0], lb[0], rb[0]})));
    r.top =
        static_cast<int32_t>(floorf(std::min({lt[1], rt[1], lb[1], rb[1]})));
    r.right =
...
Transform Transform::inverse() const {
  ...
  if (mType <= TRANSLATE) {
    ...
    if (getOrientation() & ROT_90) {
      // Recalculate the type if there is a 90-degree rotation component, since
      // the inverse of ROT_90 is ROT_270 and vice versa.
...
uint32_t Transform::getType() const { return type() & 0xFF; }

uint32_t Transform::getOrientation() const { return (type() >> 8) & 0xFF; }
