...
rcpFullResponse(float x, const ColorSpace::TransferParameters &p) {
...
static float absRcpResponse(float x, float g, float a, float b, float c,
                            float d) {
  float xx = std::abs(x);
  return std::copysign(xx >= d * c ? (std::pow(xx, 1.0f / g) - b) / a : xx / c,
                       x);
}
...
static float safePow(float x, float e) { return powf(x < 0.0f ? 0.0f : x, e); }
...
const ColorSpace ColorSpace::ProPhotoRGB() {
  return {"ROMM RGB ISO 22028-2:2013",
...
ColorSpaceConnector::ColorSpaceConnector(const ColorSpace &src,
  ...
  if (all(lessThan(abs(src.getWhitePoint() - dst.getWhitePoint()),
    ...
    if (any(greaterThan(abs(src.getWhitePoint() - ILLUMINANT_D50_XY),
                        float2{1e-3f}))) {
      rgbToXYZ =
          adaptation(BRADFORD, srcXYZ, ILLUMINANT_D50_XYZ) * src.getRGBtoXYZ();
    }

    if (any(greaterThan(abs(dst.getWhitePoint() - ILLUMINANT_D50_XY),
                        float2{1e-3f}))) {
      xyzToRGB = inverse(adaptation(BRADFORD, dstXYZ, ILLUMINANT_D50_XYZ) *
                         dst.getRGBtoXYZ());
    }
