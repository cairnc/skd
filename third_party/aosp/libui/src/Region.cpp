...
enum { direction_LTR, direction_RTL };

const Region Region::INVALID_REGION(Rect::INVALID_RECT);

// ----------------------------------------------------------------------------

Region::Region() { mStorage.push_back(Rect(0, 0)); }
...
Region::Region(const Rect &rhs) { mStorage.push_back(rhs); }
...
Region &Region::xorSelf(const Rect &r) { return operationSelf(r, op_xor); }
Region &Region::andSelf(const Rect &r) { return operationSelf(r, op_and); }
    ...
    if (prev != cur) {
      ...
      if (cur->top == prev->top) {
        if (cur->bottom != prev->bottom) {
          ...
          ALOGE_IF(!silent, "%s: spans overlap horizontally prev=%p, cur=%p",
                   name, prev, cur);
          result = false;
        }
      } else if (cur->top < prev->bottom) {
        ALOGE_IF(!silent, "%s: spans overlap vertically prev=%p, cur=%p", name,
                 prev, cur);
        result = false;
      }
      prev = cur;
...
Region::const_iterator Region::begin() const { return mStorage.data(); }
