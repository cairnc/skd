...
static void check_not_on_stack(const void *ptr) {
  ...
  if (((reinterpret_cast<uintptr_t>(ptr) ^ my_frame_address) & MIN_PAGE_MASK) ==
      0) {
  ...
  void printRefs() const {
...

    {
      ...
      if (rc >= 0) {
        ...
        ALOGE("FAILED TO PRINT STACK TRACE for %p in %s: %s", this, name,
              strerror(errno));
    }
  }
...
void RefBase::weakref_type::decWeak(const void *id) {
  ...
  LOG_ALWAYS_FATAL_IF(BAD_WEAK(c), "decWeak called on %p too many times", this);
  if (c != 1)
    return;
