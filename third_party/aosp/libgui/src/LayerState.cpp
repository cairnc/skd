...
layer_state_t::layer_state_t()
    : surface(nullptr), layerId(-1), what(0), x(0), y(0), z(0), flags(0),
      mask(0), reserved(0), cornerRadius(0.0f), backgroundBlurRadius(0),
      color(0), bufferTransform(0), transformToDisplayInverse(false),
      crop({0, 0, -1, -1}), dataspace(ui::Dataspace::UNKNOWN),
      surfaceDamageRegion(), api(-1), colorTransform(mat4()), bgColor(0),
      bgColorDataspace(ui::Dataspace::UNKNOWN), colorSpaceAgnostic(false),
      shadowRadius(0.0f), frameRateSelectionPriority(-1), frameRate(0.0f),
      ...
      frameRateSelectionStrategy(
          ANATIVEWINDOW_FRAME_RATE_SELECTION_STRATEGY_PROPAGATE),
      fixedTransformHint(ui::Transform::ROT_INVALID), autoRefresh(false),
      trustedOverlay(gui::TrustedOverlay::UNSET),
      bufferCrop(Rect::INVALID_RECT), destinationFrame(Rect::INVALID_RECT),
      dropInputMode(gui::DropInputMode::NONE),
      pictureProfileHandle(PictureProfileHandle::NONE), appContentPriority(0) {
...
status_t layer_state_t::write(Parcel &output) const {
  ...
  SAFE_PARCEL(SurfaceControl::writeNullableToParcel, output,
              relativeLayerSurfaceControl);
  SAFE_PARCEL(SurfaceControl::writeNullableToParcel, output,
              parentSurfaceControlForChild);
  ...
  for (auto listener : listeners) {
    SAFE_PARCEL(output.writeStrongBinder,
...
status_t layer_state_t::read(const Parcel &input) {
  ...
  if (hasBufferReleaseChannel) {
    bufferReleaseChannel =
        std::make_shared<gui::BufferReleaseChannel::ProducerEndpoint>();
...
status_t ComposerState::read(const Parcel &input) { return state.read(input); }
...
void layer_state_t::sanitize(int32_t permissions) {
  ...
  // arbitrary rotation. To see the problem observe that if we have a square
  // parent, and a child of the same size, then we rotate the child 45 degrees
  // around its center, the child must now be cropped to a non rectangular 8
  // sided region.
  //
  // Of course we can fix this in the future. For now, we are lucky,
  // SurfaceControl is private API, and arbitrary rotation is used in limited
  // use cases, for instance:
  // - WindowManager only uses rotation in one case, which is on a top level
  // layer in which
  //   cropping is not an issue.
  // - Launcher, as a privileged app, uses this to transition an application to
  // PiP
  //   (picture-in-picture) mode.
  //
  // However given that abuse of rotation matrices could lead to surfaces
  // extending outside of cropped areas, we need to prevent non-root clients
  // without permission ACCESS_SURFACE_FLINGER nor ROTATE_SURFACE_FLINGER
  // (a.k.a. everyone except WindowManager / tests / Launcher) from setting non
  ...
  if (what & layer_state_t::eFrameRateChanged) {
    if (!ValidateFrameRate(frameRate, frameRateCompatibility,
                           changeFrameRateStrategy, "layer_state_t::sanitize",
                           permissions & Permission::ACCESS_SURFACE_FLINGER)) {
...
void layer_state_t::merge(const layer_state_t &other) {
  ...
  if ((other.what & what) != other.what) {
    ALOGE("Unmerged SurfaceComposer Transaction properties. LayerState::merge "
          "needs updating? "
          "other.what=0x%" PRIX64 " what=0x%" PRIX64
          " unmerged flags=0x%" PRIX64,
...
uint64_t layer_state_t::diff(const layer_state_t &other) const {
  ...
  CHECK_DIFF(diff, eTransformToDisplayInverseChanged, other,
  ...
  if (other.what & eMetadataChanged)
    diff |= eMetadataChanged;
  CHECK_DIFF(diff, eShadowRadiusChanged, other, shadowRadius);
  CHECK_DIFF(diff, eDefaultFrameRateCompatibilityChanged, other,
             defaultFrameRateCompatibility);
  CHECK_DIFF(diff, eFrameRateSelectionPriority, other,
  ...
  CHECK_DIFF(diff, eFrameRateSelectionStrategyChanged, other,
             frameRateSelectionStrategy);
  ...
  if (other.what & eBufferReleaseChannelChanged)
    diff |= eBufferReleaseChannelChanged;
  if (other.what & eLutsChanged)
    diff |= eLutsChanged;
...
bool layer_state_t::hasValidBuffer() const {
  return bufferData &&
         (bufferData->hasBuffer() || bufferData->cachedBuffer.isValid());
}
...
bool InputWindowCommands::merge(const InputWindowCommands &other) {
  ...
  focusRequests.insert(focusRequests.end(),
                       std::make_move_iterator(other.focusRequests.begin()),
                       std::make_move_iterator(other.focusRequests.end()));
  changes |= !other.windowInfosReportedListeners.empty();
  windowInfosReportedListeners.insert(
      other.windowInfosReportedListeners.begin(),
...
status_t BufferData::writeToParcel(Parcel *output) const {
  ...
  SAFE_PARCEL(output->writeStrongBinder,
              IInterface::asBinder(releaseBufferListener));
...
status_t BufferData::readFromParcel(const Parcel *input) {
  ...
  if (tmpBinder) {
    releaseBufferListener =
        checked_interface_cast<ITransactionCompletedListener>(tmpBinder);
  }
