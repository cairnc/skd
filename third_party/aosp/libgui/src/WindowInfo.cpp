...
std::ostream &operator<<(std::ostream &out, const Region &region) {
  ...
  while (cur != tail) {
    ...
    out << "[" << cur->left << "," << cur->top << "][" << cur->right << ","
        << cur->bottom << "]";
    cur++;
  }
  return out;
...
bool WindowInfo::isSpy() const { return inputConfig.test(InputConfig::SPY); }
...
bool WindowInfo::overlaps(const WindowInfo *other) const {
  return !frame.isEmpty() && frame.left < other->frame.right &&
         frame.right > other->frame.left && frame.top < other->frame.bottom &&
         frame.bottom > other->frame.top;
}
...
bool WindowInfo::operator==(const WindowInfo &info) const {
  return info.token == token && info.id == id && info.name == name &&
         ...
         info.globalScaleFactor == globalScaleFactor &&
         info.transform == transform &&
         info.touchableRegion.hasSameRects(touchableRegion) &&
         info.touchOcclusionMode == touchOcclusionMode &&
         info.ownerPid == ownerPid && info.ownerUid == ownerUid &&
         info.packageName == packageName && info.inputConfig == inputConfig &&
...
status_t WindowInfo::writeToParcel(android::Parcel *parcel) const {
  ...
  // Ensure that the size of custom types are what we expect for writing into
  // the parcel.
...
status_t WindowInfo::readFromParcel(const android::Parcel *parcel) {
  ...
  dispatchingTimeout =
      static_cast<decltype(dispatchingTimeout)>(parcel->readInt64());
  ...
  int32_t lpFlags, lpType, touchOcclusionModeInt, inputConfigInt, ownerPidInt,
      ownerUidInt, displayIdInt;
...
WindowInfoHandle::WindowInfoHandle(const WindowInfoHandle &other)
    : mInfo(other.mInfo) {}
...
void WindowInfoHandle::releaseChannel() { mInfo.token.clear(); }

sp<IBinder> WindowInfoHandle::getToken() const { return mInfo.token; }
