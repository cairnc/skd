...
// layerviewer: macOS Skia has no Vulkan backend — skip GrVk / VulkanBackend.
namespace skgpu {
...
namespace {
static GrContextOptions
ganeshOptions(GrContextOptions::PersistentCache &skSLCacheMonitor) {
...
// layerviewer: Vulkan factory fatal-stubs — macOS Skia has no Vulkan backend.
std::unique_ptr<SkiaGpuContext>
SkiaGpuContext::MakeVulkan_Ganesh(const skgpu::VulkanBackendContext &,
                                  GrContextOptions::PersistentCache &) {
  LOG_ALWAYS_FATAL("MakeVulkan_Ganesh unavailable in layerviewer");
  return nullptr;
}
std::unique_ptr<SkiaGpuContext>
SkiaGpuContext::MakeVulkan_Graphite(const skgpu::VulkanBackendContext &) {
  LOG_ALWAYS_FATAL("MakeVulkan_Graphite unavailable in layerviewer");
  return nullptr;
}

GaneshGpuContext::GaneshGpuContext(sk_sp<GrDirectContext> grContext)
    : mGrContext(grContext) {
  LOG_ALWAYS_FATAL_IF(mGrContext.get() == nullptr,
...
std::unique_ptr<SkiaBackendTexture>
GaneshGpuContext::makeBackendTexture(AHardwareBuffer *buffer,
                                     bool isOutputBuffer) {
  return std::make_unique<GaneshBackendTexture>(mGrContext, buffer,
                                                isOutputBuffer);
}
...
sk_sp<SkSurface> GaneshGpuContext::createRenderTarget(SkImageInfo imageInfo) {
  ...
  return SkSurfaces::RenderTarget(mGrContext.get(), skgpu::Budgeted::kNo,
                                  imageInfo, kSampleCount,
...
void GaneshGpuContext::dumpMemoryStatistics(
