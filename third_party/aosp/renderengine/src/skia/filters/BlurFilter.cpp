...
static SkMatrix getShaderTransform(const SkCanvas *canvas,
                                   const SkRect &blurRect, const float scale) {
  // 1. Apply the blur shader matrix, which scales up the blurred surface to its
  // real size
  auto matrix = SkMatrix::Scale(scale, scale);
  // 2. Since the blurred surface has the size of the layer, we align it with
  // the top left corner of the layer position.
  matrix.postConcat(SkMatrix::Translate(blurRect.fLeft, blurRect.fTop));
  // 3. Finally, apply the inverse canvas matrix. The snapshot made in the
  // BlurFilter is in the original surface orientation. The inverse matrix has
  // to be applied to align the blur surface with the current
  // orientation/position of the canvas.
...
float BlurFilter::getMaxCrossFadeRadius() const { return mMaxCrossFadeRadius; }
...
void BlurFilter::drawBlurRegion(SkCanvas *canvas, const SkRRect &effectRegion,
  ...
  const auto blurMatrix =
      getShaderTransform(canvas, blurRect, kInverseInputScale);
  ...
  if (blurRadius < mMaxCrossFadeRadius) {
    ...
    // appropriate. In this case you might expect the matrix to simply be the
    // canvas matrix.
    ...
    blurBuilder.child("originalInput") = input->makeShader(
        SkTileMode::kClamp, SkTileMode::kClamp, linearSampling, inputMatrix);
