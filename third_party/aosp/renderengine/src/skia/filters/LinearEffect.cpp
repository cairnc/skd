...
buildRuntimeEffect(const shaders::LinearEffect &linearEffect) {
  ...
  SkString shaderString =
      SkString(shaders::buildLinearEffectSkSL(linearEffect));
...
sk_sp<SkShader> createLinearEffectShader(
  ...
  const auto uniforms = shaders::buildLinearEffectUniforms(
      linearEffect, colorTransform, maxDisplayLuminance,
      currentDisplayLuminanceNits, maxLuminance, buffer, renderIntent);

  for (const auto &uniform : uniforms) {
    effectBuilder.uniform(uniform.name.c_str())
        .set(uniform.value.data(), uniform.value.size());
  }
