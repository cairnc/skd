...
static sk_sp<SkSurface> makeSurface(SkiaGpuContext *context,
                                    const SkRect &origRect, int scale) {
  SkImageInfo scaledInfo = SkImageInfo::MakeN32Premul(
      ceil(static_cast<float>(origRect.width()) / scale),
...
void KawaseBlurDualFilter::blurInto(const sk_sp<SkSurface> &drawSurface,
  ...
  const float scale =
      static_cast<float>(drawSurface->width()) / readImage->width();
  SkMatrix blurMatrix = SkMatrix::Scale(scale, scale);
  blurInto(drawSurface,
           readImage->makeShader(
               SkTileMode::kClamp, SkTileMode::kClamp,
               SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone),
               blurMatrix),
           readImage->width() / static_cast<float>(drawSurface->width()),
           radius, alpha);
}
...
sk_sp<SkImage> KawaseBlurDualFilter::generate(SkiaGpuContext *context,
                                              const uint32_t blurRadius,
                                              const sk_sp<SkImage> input,
                                              const SkRect &blurRect) const {
  // Apply a conversion factor of (1 / sqrt(3)) to match Skia's built-in blur as
  // used by RenderEffect. See the comment in SkBlurMask.cpp for reasoning
  // behind this.
  const float radius = blurRadius * 0.57735f;

  // Use a variable number of blur passes depending on the radius. The
  // non-integer part of this calculation is used to mix the final pass into the
  // second-last with an alpha blend.
  constexpr int kMaxSurfaces = 3;
  const float filterDepth =
      std::min(kMaxSurfaces - 1.0f, radius * kInputScale / 2.5f);
  const int filterPasses =
      std::min(kMaxSurfaces - 1, static_cast<int>(ceil(filterDepth)));

  // Render into surfaces downscaled by 1x, 2x, and 4x from the initial
  // downscale.
  sk_sp<SkSurface> surfaces[kMaxSurfaces] = {
      filterPasses >= 0 ? makeSurface(context, blurRect, 1 * kInverseInputScale)
                        : nullptr,
      filterPasses >= 1 ? makeSurface(context, blurRect, 2 * kInverseInputScale)
                        : nullptr,
      filterPasses >= 2 ? makeSurface(context, blurRect, 4 * kInverseInputScale)
                        : nullptr};

  // These weights for scaling offsets per-pass are handpicked to look good at 1
  // <= radius <= 250.
  static const float kWeights[5] = {
      1.0f, // 1st downsampling pass
      1.0f, // 2nd downsampling pass
      1.0f, // 3rd downsampling pass
      0.0f, // 1st upscaling pass. Set to zero to upscale without blurring for
            // performance.
      1.0f, // 2nd upscaling pass
  };

  // Kawase is an approximation of Gaussian, but behaves differently because it
  // is made up of many simpler blurs. A transformation is required to
  // approximate the same effect as Gaussian.
  float sumSquaredR = powf(kWeights[0], 2.0f);
  for (int i = 0; i < filterPasses; i++) {
    const float alpha = std::min(1.0f, filterDepth - i);
    sumSquaredR +=
        powf(powf(2.0f, i) * alpha * kWeights[1 + i] / kInputScale, 2.0f);
    sumSquaredR +=
        powf(powf(2.0f, i + 1) * alpha * kWeights[4 - i] / kInputScale, 2.0f);
  }
  ...
  {
    // For sampling Skia's API expects the inverse of what logically seems
    // appropriate. In this case one may expect Translate(blurRect.fLeft,
    // blurRect.fTop) * Scale(kInverseInputScale) but instead we must do the
    // inverse.
    SkMatrix blurMatrix = SkMatrix::Translate(-blurRect.fLeft, -blurRect.fTop);
    blurMatrix.postScale(kInputScale, kInputScale);
    const auto sourceShader = input->makeShader(
        SkTileMode::kClamp, SkTileMode::kClamp,
        SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone),
        blurMatrix);
    blurInto(surfaces[0], std::move(sourceShader), kInputScale,
             kWeights[0] * step, 1.0f);
  }
  // Next the remaining downscale blur passes.
  for (int i = 0; i < filterPasses; i++) {
    blurInto(surfaces[i + 1], surfaces[i]->makeImageSnapshot(),
             kWeights[1 + i] * step, 1.0f);
  }
  // Finally blur+upscale back to our original size.
  for (int i = filterPasses - 1; i >= 0; i--) {
    blurInto(surfaces[i], surfaces[i + 1]->makeImageSnapshot(),
             kWeights[4 - i] * step, std::min(1.0f, filterDepth - i));
  }
