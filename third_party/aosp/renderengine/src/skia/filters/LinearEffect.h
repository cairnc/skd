...
buildRuntimeEffect(const shaders::LinearEffect &linearEffect);
...
// * The max display luminance is the max luminance of the physical display in
// nits
// * The current luminance of the physical display in nits
// * The max luminance is provided as the max luminance for the buffer, either
// from the SMPTE 2086 or as the max light level from the CTA 861.3 standard.
// * An AHardwareBuffer for implementations that support gralloc4 metadata for
// communicating any HDR metadata.
// * A RenderIntent that communicates the downstream renderintent for a physical
// display, for image quality compensation.
