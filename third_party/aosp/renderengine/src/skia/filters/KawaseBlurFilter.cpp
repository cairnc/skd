...
// Draws the given runtime shader on a GPU (Ganesh) surface and returns the
// result as an SkImage.
static sk_sp<SkImage> makeImage(SkSurface *surface,
...
sk_sp<SkImage> KawaseBlurFilter::generate(SkiaGpuContext *context,
  ...
  // A radius transformation is required for approximating them, and also to
  // introduce non-integer steps, necessary to smoothly interpolate large radii.
  float tmpRadius = (float)blurRadius / 2.0f;
  uint32_t numberOfPasses = std::min(kMaxPasses, (uint32_t)ceil(tmpRadius));
  float radiusByPasses = tmpRadius / (float)numberOfPasses;

  // create blur surface with the bit depth and colorspace of the original
  // surface
  SkImageInfo scaledInfo =
      input->imageInfo().makeWH(std::ceil(blurRect.width() * kInputScale),
                                std::ceil(blurRect.height() * kInputScale));

  // For sampling Skia's API expects the inverse of what logically seems
  // appropriate. In this case you might expect Translate(blurRect.fLeft,
  // blurRect.fTop) X Scale(kInverseInputScale) but instead we must do the
  // inverse.
  ...
  blurBuilder.child("child") = input->makeShader(
      SkTileMode::kClamp, SkTileMode::kClamp, linear, blurMatrix);
  blurBuilder.uniform("in_blurOffset") = radiusByPasses * kInputScale;

  sk_sp<SkSurface> surface = context->createRenderTarget(scaledInfo);
  LOG_ALWAYS_FATAL_IF(!surface, "%s: Failed to create surface for blurring!",
                      __func__);
  sk_sp<SkImage> tmpBlur = makeImage(surface.get(), &blurBuilder);

  // And now we'll build our chain of scaled blur stages. If there is more than
  // one pass, create a second surface and ping pong between them.
  ...
  if (numberOfPasses <= 1) {
    ...
    LOG_ALWAYS_FATAL_IF(!surfaceTwo,
                        "%s: Failed to create second blur surface!", __func__);
    ...
    for (auto i = 1; i < numberOfPasses; i++) {
      ...
      blurBuilder.uniform("in_blurOffset") =
          (float)i * radiusByPasses * kInputScale;
