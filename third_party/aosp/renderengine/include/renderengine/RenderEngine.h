...
/**
 * Allows opting particular devices into an initial preview rollout of
 * RenderEngine on Graphite.
 *
 * Only applicable within SurfaceFlinger, and if relevant aconfig flags are
 * enabled.
 */
...
/**
 * Turns on recording of skia commands in SkiaGL version of the RE. This
 * property defines number of milliseconds for the recording to take place. A
 * non zero value turns on the recording.
 */
#define PROPERTY_DEBUG_RENDERENGINE_CAPTURE_SKIA_MS                            \
  "debug.renderengine.capture_skia_ms"

/**
 * Set to the most recently saved file once the capture is finished.
 */
#define PROPERTY_DEBUG_RENDERENGINE_CAPTURE_FILENAME                           \
  "debug.renderengine.capture_filename"

/**
 * Switches the cross-window background blur algorithm.
 */
#define PROPERTY_DEBUG_RENDERENGINE_BLUR_ALGORITHM                             \
  "debug.renderengine.blur_algorithm"
...
public:
  ...
  //
  // If called for GraphicsApi::VK then underlying (unprotected) VK resources
  // will be preserved to optimize subsequent VK initialization, but
  // teardown(GraphicsApi::VK) must be invoked if the caller subsequently
  // decides to NOT use VK.
  //
  // The first call may require significant resource initialization, but
  // subsequent checks are cached internally.
  static bool canSupport(GraphicsApi graphicsApi);

  // Teardown any GPU API resources that were previously initialized but are no
  // longer needed.
  //
  // Must be called with GraphicsApi::VK if canSupport(GraphicsApi::VK) was
  // previously invoked but the caller subsequently decided to not use VK.
  //
  // This is safe to call if there is nothing to teardown, but NOT safe to call
  // if a RenderEngine instance exists. The RenderEngine destructor will handle
  // its own teardown logic.
  ...
  virtual ftl::Future<FenceResult>
  drawLayers(const DisplaySettings &display,
             const std::vector<LayerSettings> &layers,
             const std::shared_ptr<ExternalTexture> &buffer,
             base::unique_fd &&bufferFence);

  virtual ftl::Future<FenceResult> drawGainmap(
  ...
  // Returns true if blur was requested in the RenderEngineCreationArgs and the
  // implementation also supports background blur.  If false, no blur will be
  // applied when drawing layers. This query is required to be thread safe.
  virtual bool supportsBackgroundBlur() = 0;

  // TODO(b/180767535): This is only implemented to allow for backend-specific
  // behavior, which we should not allow in general, so remove this.
  ...
  // Allows flinger to get the render engine thread id for power management with
  // ADPF Returns the tid of the renderengine thread if it's threaded, and
  // std::nullopt otherwise
  virtual std::optional<pid_t> getRenderEngineTid() const {
    return std::nullopt;
  }
...
protected:
  ...
  // Note also that if the buffer contains protected content, then mapping those
  // GPU resources may be deferred until the buffer is really used for drawing.
  // This is because typical SoCs that support protected memory only support a
  // limited amount, so optimisitically mapping protected memory may be too
  // burdensome. If a buffer contains protected content and the RenderEngine
  // implementation supports protected context, then GPU resources may be mapped
  // into both the protected and unprotected contexts. If the buffer may ever be
  // written to by RenderEngine, then isRenderable must be true.
  ...
  // Note that if there are multiple callers holding onto the same buffer, then
  // the buffer's resources may be internally ref-counted to guard against
  // use-after-free errors. Note that work may be deferred to an additional
  // thread, i.e. this call is expected to be made asynchronously, but the
  // caller can expect that map/unmap calls are performed in a manner that's
  // conflict serializable, i.e. unmap a buffer should never occur before
  // binding the buffer if the caller called mapExternalTextureBuffer before
  // calling unmap.
  virtual void unmapExternalTextureBuffer(sp<GraphicBuffer> &&buffer) = 0;

  // A thread safe query to determine if any post rendering cleanup is
  // necessary.  Returning true is a signal that calling the postRenderCleanup
  // method would be a no-op and that callers can avoid any thread
  // synchronization that may be required by directly calling postRenderCleanup.
  ...
  // Update protectedContext mode depending on whether or not any layer has a
  // protected buffer.
...
private:
  ...
  RenderEngineCreationArgs(int _pixelFormat, uint32_t _imageCacheSize,
      ...
      : pixelFormat(_pixelFormat), imageCacheSize(_imageCacheSize),
        ...
        blurAlgorithm(_blurAlgorithm), contextPriority(_contextPriority),
        threaded(_threaded), graphicsApi(_graphicsApi),
...
struct RenderEngineCreationArgs::Builder {
  ...
  RenderEngineCreationArgs build() const {
    return RenderEngineCreationArgs(
        pixelFormat, imageCacheSize, enableProtectedContext,
        precacheToneMapperShaderOnly, blurAlgorithm, contextPriority, threaded,
        graphicsApi, skiaBackend);
  }
...
private:
  ...
  RenderEngine::ContextPriority contextPriority =
      RenderEngine::ContextPriority::MEDIUM;
