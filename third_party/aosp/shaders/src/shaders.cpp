...
toAidlDataspace(ui::Dataspace dataspace) {
  return static_cast<aidl::android::hardware::graphics::common::Dataspace>(
      dataspace);
}
...
void generateLuminanceScalesForOOTF(ui::Dataspace inputDataspace,
                                    std::string &shader) {
  switch (inputDataspace & HAL_DATASPACE_TRANSFER_MASK) {
  case HAL_DATASPACE_TRANSFER_HLG:
    // BT. 2408 says that a signal level of 0.75 == 203 nits for HLG, but that's
    // after applying OOTF. But we haven't applied OOTF yet, so we need to scale
    // by a different constant instead.
    shader.append(R"(
...
// Normalizes from absolute light back to relative light (maps from [0, maxNits]
// back to [0, 1])
static void generateLuminanceNormalizationForOOTF(ui::Dataspace inputDataspace,
  ...
  case HAL_DATASPACE_TRANSFER_HLG:
    ...
    default:
      ...
      // BT. 2100-2 operates on normalized luminances, so renormalize to the
      // input to correctly adjust gamma. Note that following BT. 2408 for HLG
      // OETF actually maps 0.75 == ~264.96 nits, rather than 203 nits, because
      // 203 nits == OOTF(invOETF(0.75)), so even though we originally scaled by
      // 203 nits we need to re-normalize to 264.96 nits when converting to the
      // correct brightness range.
      shader.append(R"(
                            float3 NormalizeLuminance(float3 xyz) {
...
void generateOOTF(ui::Dataspace inputDataspace, ui::Dataspace outputDataspace,
  ...
  generateLuminanceNormalizationForOOTF(inputDataspace, outputDataspace,
                                        shader);

  // Some tonemappers operate on CIE luminance, other tonemappers operate on
  // linear rgb luminance in the source gamut.
  shader.append(R"(
...
void generateEffectiveOOTF(bool undoPremultipliedAlpha,
                           LinearEffect::SkSLType type, bool needsCustomOETF,
                           std::string &shader) {
  switch (type) {
...
std::string buildLinearEffectSkSL(const LinearEffect &linearEffect) {
  ...
  generateOOTF(linearEffect.inputDataspace, linearEffect.outputDataspace,
               shaderString);

  const bool needsCustomOETF =
      (linearEffect.fakeOutputDataspace & HAL_DATASPACE_TRANSFER_MASK) ==
      HAL_DATASPACE_TRANSFER_GAMMA2_2;
  if (needsCustomOETF) {
    generateOETF(shaderString);
  }
  generateEffectiveOOTF(linearEffect.undoPremultipliedAlpha, linearEffect.type,
                        needsCustomOETF, shaderString);
  return shaderString;
}
...
std::vector<tonemap::ShaderUniform> buildLinearEffectUniforms(
    ...
    float maxLuminance, AHardwareBuffer *buffer,
  ...
  uniforms.push_back({.name = "in_rgbToXyz",
                      .value = buildUniformValue<mat3>(
                          ColorSpace::linearExtendedSRGB().getRGBtoXYZ())});
  uniforms.push_back(
      {.name = "in_xyzToSrcRgb",
       .value = buildUniformValue<mat3>(inputColorSpace.getXYZtoRGB())});
  // Transforms xyz colors to linear source colors, then applies the color
  // transform, then transforms to linear extended RGB for skia to color manage.
  uniforms.push_back(
  ...
  tonemap::Metadata metadata{
      .displayMaxLuminance = maxDisplayLuminance,
  ...
  for (const auto uniform :
       tonemap::getToneMapper()->generateShaderSkSLUniforms(metadata)) {
