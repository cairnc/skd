...
public:
  // Utility class to map handles to ids and buffers to buffer properties
  // without pulling in SurfaceFlinger dependencies.
  class FlingerDataMapper {
  public:
    virtual ~FlingerDataMapper() = default;
    virtual sp<IBinder> getDisplayHandle(int32_t /* displayId */) const {
      return nullptr;
    }
    virtual int32_t
    getDisplayId(const sp<IBinder> & /* displayHandle */) const {
      return -1;
    }
  };
  ...
  toProto(const std::map<uint32_t /* layerId */, TracingLayerState> &);
  ...
  static perfetto::protos::DisplayInfo toProto(const frontend::DisplayInfo &,
                                               uint32_t layerStack);

  TransactionState fromProto(const perfetto::protos::TransactionState &);
  void mergeFromProto(const perfetto::protos::LayerState &,
                      TracingLayerState &outState);
  void fromProto(const perfetto::protos::LayerCreationArgs &,
...
private:
  ...
  void fromProto(const perfetto::protos::LayerState &,
                 ResolvedComposerState &out);
