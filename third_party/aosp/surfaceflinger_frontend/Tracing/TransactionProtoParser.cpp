...
TransactionProtoParser::toProto(const TransactionState &t) {
  ...
  proto.mutable_display_changes()->Reserve(
      static_cast<int32_t>(t.displays.size()));
...
perfetto::protos::LayerState TransactionProtoParser::toProto(
  ...
  if (layer.what & layer_state_t::eTransparentRegionChanged) {
    LayerProtoHelper::writeToProto(layer.transparentRegion,
  ...
  if (layer.what & layer_state_t::eBufferChanged) {
    perfetto::protos::LayerState_BufferData *bufferProto =
        proto.mutable_buffer_data();
    if (resolvedComposerState.externalTexture) {
      bufferProto->set_buffer_id(
          resolvedComposerState.externalTexture->getId());
      bufferProto->set_width(resolvedComposerState.externalTexture->getWidth());
      bufferProto->set_height(
          resolvedComposerState.externalTexture->getHeight());
  ...
  if (layer.what & layer_state_t::eColorTransformChanged) {
    LayerProtoHelper::writeToProto(layer.colorTransform,
                                   proto.mutable_color_transform());
  }
  ...
  if (layer.what & layer_state_t::eInputInfoChanged) {
    if (layer.windowInfoHandle) {
      ...
      windowInfoProto->set_layout_params_flags(
          inputInfo->layoutParamsFlags.get());
      ...
      windowInfoProto->set_focusable(!inputInfo->inputConfig.test(
      ...
      perfetto::protos::Transform *transformProto =
          windowInfoProto->mutable_transform();
  ...
  if (layer.what & layer_state_t::eTrustedOverlayChanged) {
    proto.set_is_trusted_overlay(layer.trustedOverlay ==
                                 gui::TrustedOverlay::ENABLED);
    // TODO(b/339701674) update protos
  }
  if (layer.what & layer_state_t::eBufferCropChanged) {
    LayerProtoHelper::writeToProto(layer.bufferCrop,
                                   proto.mutable_buffer_crop());
  }
  if (layer.what & layer_state_t::eDestinationFrameChanged) {
    LayerProtoHelper::writeToProto(layer.destinationFrame,
                                   proto.mutable_destination_frame());
  }
  if (layer.what & layer_state_t::eDropInputModeChanged) {
    proto.set_drop_input_mode(
        static_cast<perfetto::protos::LayerState_DropInputMode>(
            layer.dropInputMode));
  }
  return proto;
}
...
TransactionProtoParser::toProto(const DisplayState &display) {
...
TransactionProtoParser::toProto(const LayerCreationArgs &args) {
...
void TransactionProtoParser::fromProto(
    const perfetto::protos::LayerCreationArgs &proto,
...
void TransactionProtoParser::mergeFromProto(
    const perfetto::protos::LayerState &proto, TracingLayerState &outState) {
...
void TransactionProtoParser::fromProto(
    const perfetto::protos::LayerState &proto,
    ResolvedComposerState &resolvedComposerState) {
  auto &layer = resolvedComposerState.state;
  ...
  if (proto.what() & layer_state_t::eTransparentRegionChanged) {
    LayerProtoHelper::readFromProto(proto.transparent_region(),
                                    layer.transparentRegion);
  }
  ...
  if (proto.what() & layer_state_t::eBufferChanged) {
    const perfetto::protos::LayerState_BufferData &bufferProto =
        proto.buffer_data();
    layer.bufferData = std::make_shared<fake::BufferData>(
        bufferProto.buffer_id(), bufferProto.width(), bufferProto.height(),
        bufferProto.pixel_format(), bufferProto.usage());
    resolvedComposerState.externalTexture =
        std::make_shared<FakeExternalTexture>(
            layer.bufferData->getWidth(), layer.bufferData->getHeight(),
            layer.bufferData->getId(), layer.bufferData->getPixelFormat(),
            layer.bufferData->getUsage());
    layer.bufferData->frameNumber = bufferProto.frame_number();
    layer.bufferData->flags =
        ftl::Flags<BufferData::BufferDataChange>(bufferProto.flags());
  ...
  if (proto.what() & layer_state_t::eColorTransformChanged) {
    LayerProtoHelper::readFromProto(proto.color_transform(),
  ...
  if ((proto.what() & layer_state_t::eInputInfoChanged) &&
      proto.has_window_info_handle()) {
    gui::WindowInfo inputInfo;
    const perfetto::protos::LayerState_WindowInfo &windowInfoProto =
        proto.window_info_handle();

    inputInfo.layoutParamsFlags = static_cast<gui::WindowInfo::Flag>(
        windowInfoProto.layout_params_flags());
    inputInfo.layoutParamsType = static_cast<gui::WindowInfo::Type>(
        windowInfoProto.layout_params_type());
    LayerProtoHelper::readFromProto(windowInfoProto.touchable_region(),
                                    inputInfo.touchableRegion);
    inputInfo.inputConfig = ftl::Flags<gui::WindowInfo::InputConfig>(
        windowInfoProto.input_config());
    inputInfo.surfaceInset = windowInfoProto.surface_inset();
    inputInfo.globalScaleFactor = windowInfoProto.global_scale_factor();
    const perfetto::protos::Transform &transformProto =
        windowInfoProto.transform();
    inputInfo.transform.set(transformProto.dsdx(), transformProto.dtdx(),
                            transformProto.dtdy(), transformProto.dsdy());
  ...
  if (proto.what() & layer_state_t::eBackgroundColorChanged) {
    ...
    layer.bgColorDataspace =
        static_cast<ui::Dataspace>(proto.bg_color_dataspace());
    const perfetto::protos::LayerState_Color3 &colorProto = proto.color();
  ...
  if (proto.what() & layer_state_t::eFrameRateChanged) {
    ...
    layer.frameRateCompatibility =
        static_cast<int8_t>(proto.frame_rate_compatibility());
    layer.changeFrameRateStrategy =
        static_cast<int8_t>(proto.change_frame_rate_strategy());
  }
  ...
  if (proto.what() & layer_state_t::eTrustedOverlayChanged) {
    layer.trustedOverlay = proto.is_trusted_overlay()
                               ? gui::TrustedOverlay::ENABLED
  ...
  if (proto.what() & layer_state_t::eDestinationFrameChanged) {
    LayerProtoHelper::readFromProto(proto.destination_frame(),
                                    layer.destinationFrame);
  }
  if (proto.what() & layer_state_t::eDropInputModeChanged) {
    layer.dropInputMode =
        static_cast<gui::DropInputMode>(proto.drop_input_mode());
  }
}

DisplayState
TransactionProtoParser::fromProto(const perfetto::protos::DisplayState &proto) {
...
void asProto(perfetto::protos::Transform *proto,
...
TransactionProtoParser::toProto(const frontend::DisplayInfo &displayInfo,
                                uint32_t layerStack) {
...
void fromProto2(ui::Transform &outTransform,
                const perfetto::protos::Transform &proto) {
  outTransform.set(proto.dsdx(), proto.dtdx(), proto.dtdy(), proto.dsdy());
  outTransform.set(proto.tx(), proto.ty());
}

frontend::DisplayInfo
TransactionProtoParser::fromProto(const perfetto::protos::DisplayInfo &proto) {
  ...
  displayInfo.rotationFlags =
      (ui::Transform::RotationFlags)proto.rotation_flags();
  displayInfo.transformHint =
      (ui::Transform::RotationFlags)proto.transform_hint();
  return displayInfo;
}

void TransactionProtoParser::fromProto(
    const google::protobuf::RepeatedPtrField<perfetto::protos::DisplayInfo>
        &proto,
    frontend::DisplayInfos &outDisplayInfos) {
  outDisplayInfos.clear();
  for (const perfetto::protos::DisplayInfo &displayInfo : proto) {
    outDisplayInfos.emplace_or_replace(
        ui::LayerStack::fromValue(displayInfo.layer_stack()),
