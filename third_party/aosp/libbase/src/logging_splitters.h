...
static std::string StderrOutputGenerator(const struct timespec &ts, int pid,
  ...
  if (file != nullptr) {
    line_prefix = StringPrintf("%s %5d %5" PRIu64 " %c %-8s: %s:%u ", timestamp,
                               pid, tid, severity_char, real_tag, file, line);
  } else {
    line_prefix = StringPrintf("%s %5d %5" PRIu64 " %c %-8s: ", timestamp, pid,
                               tid, severity_char, real_tag);
  }
