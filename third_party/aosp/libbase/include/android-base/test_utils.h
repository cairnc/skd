...
#define __LIBBASE_GENERIC_REGEX_SEARCH(__s, __pattern)                         \
  (std::regex_search(                                                          \
      __s, std::basic_regex<std::decay<decltype(__s[0])>::type>((__pattern))))
...
#define EXPECT_NOT_MATCH(__string, __pattern)                                  \
  do {                                                                         \
    auto __s = (__string);                                                     \
    if (__LIBBASE_GENERIC_REGEX_SEARCH(__s, (__pattern))) {                    \
      ADD_FAILURE() << "regex mismatch: expected to not find " << (__pattern)  \
                    << " in:\n"                                                \
                    << __s;                                                    \
    }                                                                          \
  } while (0)
...
static inline bool running_with_hwasan() { return &__hwasan_init != 0; }
