...
public:
  ...
  void invoke(bool presentedWithinThresholds) {
    callbackInterface->onTrustedPresentationChanged(callbackId,
...
public:
  ...
  // Stores which endpoint the release information should be sent to. We don't
  // want to send the releaseCallbackId and release fence to all listeners so we
  // store which listener the setBuffer was called with.
...
struct layer_state_t {
  ...
  enum {
    ...
    // Queue up layer buffers instead of dropping the oldest buffer when this
    // flag is set. This blocks the client until all the buffers have been
    // presented. If the buffers have presentation timestamps, then we may drop
    // buffers.
    eEnableBackpressure = 0x100,       // ENABLE_BACKPRESSURE
    eLayerIsDisplayDecoration = 0x200, // DISPLAY_DECORATION
    // Ignore any destination frame set on the layer. This is used when the
    // buffer scaling mode is freeze and the destination frame is applied
    // asynchronously with the buffer submission. This is needed to maintain
    // compatibility for SurfaceView scaling behavior. See SurfaceView scaling
    // behavior for more details.
    eIgnoreDestinationFrame = 0x400,
    eLayerIsRefreshRateIndicator = 0x800, // REFRESH_RATE_INDICATOR
    // Sets a property on this layer indicating that its visible region should
    // be considered when computing TrustedPresentation Thresholds.
    eCanOccludePresentation = 0x1000,
    // Indicates that the SurfaceControl should recover from buffer stuffing
    // when possible. This is the case when the SurfaceControl is the root
    // SurfaceControl owned by ViewRootImpl.
  ...
  // Compares two layer_state_t structs and returns a set of change flags
  // describing all the states that are different.
  ...
  static constexpr uint64_t HIERARCHY_CHANGES =
      ...
      layer_state_t::eReparent | layer_state_t::eLayerStackChanged;

  // Geometry updates.
  static constexpr uint64_t GEOMETRY_CHANGES =
      layer_state_t::eBufferCropChanged |
      layer_state_t::eBufferTransformChanged |
      layer_state_t::eCornerRadiusChanged | layer_state_t::eCropChanged |
      layer_state_t::eDestinationFrameChanged | layer_state_t::eMatrixChanged |
      layer_state_t::ePositionChanged |
  ...
  static constexpr uint64_t BUFFER_CHANGES =
      layer_state_t::eApiChanged | layer_state_t::eBufferChanged |
      layer_state_t::eBufferCropChanged |
      layer_state_t::eBufferTransformChanged |
      layer_state_t::eDataspaceChanged | layer_state_t::eSidebandStreamChanged |
  ...
  static constexpr uint64_t CONTENT_CHANGES =
      layer_state_t::BUFFER_CHANGES | layer_state_t::eAlphaChanged |
      ...
      layer_state_t::ePictureProfileHandleChanged |
      layer_state_t::eAppContentPriorityChanged;

  // Changes which invalidates the layer's visible region in CE.
  static constexpr uint64_t CONTENT_DIRTY = layer_state_t::CONTENT_CHANGES |
                                            layer_state_t::GEOMETRY_CHANGES |
                                            layer_state_t::HIERARCHY_CHANGES;

  // Changes affecting child states.
  static constexpr uint64_t AFFECTS_CHILDREN =
      ...
      layer_state_t::eColorTransformChanged |
      layer_state_t::eCornerRadiusChanged | layer_state_t::eFlagsChanged |
      layer_state_t::eTrustedOverlayChanged | layer_state_t::eFrameRateChanged |
      layer_state_t::eFrameRateCategoryChanged |
      layer_state_t::eFrameRateSelectionStrategyChanged |
      layer_state_t::eFrameRateSelectionPriority |
      layer_state_t::eFixedTransformHintChanged;

  // Changes affecting data sent to input.
  static constexpr uint64_t INPUT_CHANGES =
      layer_state_t::eAlphaChanged | layer_state_t::eInputInfoChanged |
      layer_state_t::eDropInputModeChanged |
  ...
  sp<gui::WindowInfoHandle> windowInfoHandle =
  ...
  // Default frame rate compatibility used to set the layer refresh rate
  // votetype.
  ...
  // An inherited state that indicates that this surface control and its
  // children should be trusted for input occlusion detection purposes
  ...
  // Enhance the quality of the buffer contents by configurating a picture
  // processing pipeline with values as specified by this picture profile.
  PictureProfileHandle pictureProfileHandle{PictureProfileHandle::NONE};

  // A value indicating the significance of the layer's content to the app's
  // desired user experience. A lower priority will result in more likelihood of
  // getting access to limited resources, such as picture processing hardware.
  ...
  std::shared_ptr<gui::BufferReleaseChannel::ProducerEndpoint>
...
struct DisplayState {
  ...
  DisplayState(sp<IBinder> token, ui::LayerStack layerStack)
      : what(eAllChanged), token(std::move(token)), layerStack(layerStack),
  ...
  // These states define how layers are projected onto the physical or virtual
  // display.
  //
  // Layers are first clipped to `layerStackSpaceRect'.  They are then
  // translated and scaled from `layerStackSpaceRect' to
  // `orientedDisplaySpaceRect'.  Finally, they are rotated according to
  // `orientation', `width', and `height'.
  //
  // For example, assume layerStackSpaceRect is Rect(0, 0, 200, 100),
  // orientedDisplaySpaceRect is Rect(20, 10, 420, 210), and the size of the
  // display is WxH.  When orientation is 0, layers will be scaled by a factor
  // of 2 and translated by (20, 10). When orientation is 1, layers will be
  // additionally rotated by 90 degrees around the origin clockwise and
  // translated by (W, 0).
  //
  // Rect::INVALID_RECT sizes the space to the active resolution of the physical
  // display, or the default dimensions of the virtual display surface.
  //
  ui::Rotation orientation = ui::ROTATION_0;
  Rect layerStackSpaceRect = Rect::EMPTY_RECT;
  Rect orientedDisplaySpaceRect = Rect::EMPTY_RECT;

  // Exclusive to virtual displays: The sink surface into which the virtual
  // display is rendered, and an optional resolution that overrides its default
  // dimensions.
...
static inline int compare_type(const ComposerState &lhs,
                               const ComposerState &rhs) {
  if (lhs.state.surface < rhs.state.surface)
    return -1;
  if (lhs.state.surface > rhs.state.surface)
    return 1;
  return 0;
...
static inline int compare_type(const DisplayState &lhs,
                               const DisplayState &rhs) {
