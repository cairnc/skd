...
#undef FIRST_SYSTEM_WINDOW_
  ...
  // type. This indicates that they are flags, so it can be used with
  // ftl/enum.h.
   ...
   */
  // This value should NOT be used to uniquely identify the window. There may be
  // different input windows that have the same token.
  sp<IBinder> token;

  // The token that identifies which client window this WindowInfo was created
  // for.
  ...
  // The real size of the content, excluding any crop. If no buffer is rendered,
  // this is 0,0
  ui::Size contentSize = ui::Size(0, 0);

  /*
   * SurfaceFlinger consumes this value to shrink the computed frame. This is
   * different from shrinking the touchable region in that it DOES shift the
   * coordinate space where-as the touchable region does not and is more like
   * "cropping". This is used for window shadows.
   */
  ...
  // The input token for the window to which focus should be transferred when
  // this input window can be successfully focused. If null, this input window
  // will not transfer its focus to any other window.
  sp<IBinder> focusTransferTarget;

  // Sets a property on this window indicating that its visible region should be
  // considered when computing TrustedPresentation Thresholds.
...
public:
  ...
  getDispatchingTimeout(std::chrono::nanoseconds defaultValue) const {
    return mInfo.token ? std::chrono::nanoseconds(mInfo.dispatchingTimeout)
                       : defaultValue;
  }
