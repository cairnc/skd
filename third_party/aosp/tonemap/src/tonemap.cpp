...
static const constexpr auto kTransferMask = static_cast<int32_t>(
    aidl::android::hardware::graphics::common::Dataspace::TRANSFER_MASK);
static const constexpr auto kTransferST2084 = static_cast<int32_t>(
    aidl::android::hardware::graphics::common::Dataspace::TRANSFER_ST2084);
static const constexpr auto kTransferHLG = static_cast<int32_t>(
...
float computeHlgGamma(float currentDisplayBrightnessNits) {
  ...
  currentDisplayBrightnessNits =
      std::max(minBrightnessNits, currentDisplayBrightnessNits);
...
public:
  std::string generateTonemapGainShaderSkSL(
    ...
    const int32_t destinationDataspaceInt =
        static_cast<int32_t>(destinationDataspace);
  ...
  generateShaderSkSLUniforms(const Metadata &metadata) override {
    std::vector<ShaderUniform> uniforms;

    uniforms.reserve(2);

    uniforms.push_back(
        {.name = "in_libtonemap_displayMaxLuminance",
         .value = buildUniformValue<float>(metadata.displayMaxLuminance)});
    uniforms.push_back(
        {.name = "in_libtonemap_inputMaxLuminance",
    ...
    for (const auto [_, xyz] : colors) {
      ...
      case kTransferHLG:
        ...
        case kTransferST2084:
          ...
          break;
        case kTransferHLG:
          // PQ has a wider luminance range (10,000 nits vs. 1,000 nits) than
          // HLG, so we'll clamp the luminance range in case we're mapping from
          // PQ input to HLG output.
      ...
      default:
        ...
        default:
          // For completeness, this is tone-mapping from SDR to SDR, where this
          // is just a no-op.
          targetNits = xyz.y;
          break;
        }
...
private:
  ...
  double OETF_HLG(double nits) {
    ...
    return nits <= 1.0 / 12.0 ? std::sqrt(3.0 * nits)
                              : a * std::log(12.0 * nits - b) + c;
  }
...
public:
  std::string generateTonemapGainShaderSkSL(
    ...
    case kTransferHLG:
      ...
      default:
        ...
        // tone-mapping down to SDR, as libshaders normalizes all SDR output
        // from [0, maxDisplayLumins] -> [0, 1]
        program.append(R"(
                                    float libtonemap_ToneMapTargetNits(float maxRGB) {
                                        return maxRGB
                                                * pow(maxRGB / 1000.0, in_libtonemap_hlgGamma - 1)
  ...
  generateShaderSkSLUniforms(const Metadata &metadata) override {
    ...
    uniforms.push_back({.name = "in_libtonemap_hlgGamma",
                        .value = buildUniformValue<float>(computeHlgGamma(
                            metadata.currentDisplayLuminance))});
    return uniforms;
  }
