...
template <typename... Ms> struct Matcher : Ms... {
  using Ms::operator()...;
};

// Deduction guide.
template <typename... Ms> Matcher(Ms...) -> Matcher<Ms...>;

template <typename Matcher, typename... Ts>
constexpr bool is_exhaustive_match_v =
    (std::is_invocable_v<Matcher, Ts> && ...);

template <typename...> struct Match;

template <typename T, typename U, typename... Ts> struct Match<T, U, Ts...> {
...
template <typename T> struct Match<T> {
  template <typename Variant, typename Matcher>
  static decltype(auto) match(Variant &variant, const Matcher &matcher) {
