...
template <typename T, typename = void> struct StaticString;

// Booleans.
template <typename T> struct StaticString<T, std::enable_if_t<is_bool_v<T>>> {
...
// Characters.
template <typename T> struct StaticString<T, std::enable_if_t<is_char_v<T>>> {
...
template <typename T>
struct StaticString<T, std::enable_if_t<std::is_integral_v<remove_cvref_t<T>> &&
                                        !is_bool_v<T> && !is_char_v<T>>> {
...
// Character arrays.
template <std::size_t M> struct StaticString<const char (&)[M], void> {
...
template <std::size_t N> struct Truncated {
  std::string_view view;
};

// Strings with constrained length.
template <std::size_t M> struct StaticString<Truncated<M>, void> {
  static constexpr std::size_t N = M;

  explicit constexpr StaticString(Truncated<M> str)
      : view(str.view.substr(0, N)) {}
