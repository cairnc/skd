...
template <typename T> struct ArrayTraits {
  ...
  // TODO: Make constexpr in C++20.
  static void in_place_swap_ranges(iterator first1, iterator last1,
                                   iterator first2) {
    while (first1 != last1) {
  ...
  static void uninitialized_copy(Iterator first, Iterator last,
                                 const_iterator out) {
    while (first != last) {
...
// CRTP mixin to define iterator functions in terms of non-const Self::begin and
// Self::end.
template <typename Self, typename T> class ArrayIterators {
