...
template <typename T = std::uint64_t> inline T read_unaligned(const void *ptr) {
  T v;
...
constexpr std::uint64_t shift_mix(std::uint64_t v) { return v ^ (v >> 47); }

__attribute__((
    no_sanitize("unsigned-integer-overflow"))) constexpr std::uint64_t
hash_length_16(std::uint64_t u, std::uint64_t v) {
...
__attribute__((no_sanitize("unsigned-integer-overflow"))) inline std::uint64_t
hash_length_0_to_16(const char *str, std::uint64_t length) {
  if (length > 8) {
    const auto a = read_unaligned(str);
    const auto b = read_unaligned(str + length - 8);
    return hash_length_16(
               a, rotate<true>(b + length, static_cast<std::uint8_t>(length))) ^
           b;
  }
  if (length >= 4) {
  ...
  if (length > 0) {
    ...
    const auto y =
        static_cast<std::uint32_t>(a) + (static_cast<std::uint32_t>(b) << 8);
...
__attribute__((no_sanitize("unsigned-integer-overflow"))) inline std::uint64_t
hash_length_17_to_32(const char *str, std::uint64_t length) {
...
__attribute__((no_sanitize("unsigned-integer-overflow"))) inline std::uint64_t
hash_length_33_to_64(const char *str, std::uint64_t length) {
  auto z = read_unaligned(str + 24);
  auto a = read_unaligned(str) +
           (length + read_unaligned(str + length - 16)) * kPrime0;
