...
template <typename, template <typename> class> class Future;
...
template <typename T> struct future_result<std::future<T>> {
  using type = T;
};

template <typename T> struct future_result<std::shared_future<T>> {
  using type = T;
};
...
template <typename, typename T, template <typename> class> class BaseFuture;

template <typename Self, typename T> class BaseFuture<Self, T, std::future> {
