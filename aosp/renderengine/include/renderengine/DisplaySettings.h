...
struct DisplaySettings {
  ...
  // All layers will be dimmed by (max(layer white points) /
  // targetLuminanceNits). If the target luminance is unknown, then no
  // display-level dimming occurs.
  ...
  // Configures the rendering intent of the output display. This is used for
  // tonemapping.
  aidl::android::hardware::graphics::composer3::RenderIntent renderIntent =
      aidl::android::hardware::graphics::composer3::RenderIntent::
          TONE_MAP_COLORIMETRIC;

  // Tonemapping strategy to use for each layer. This is only used for
  // tonemapping HDR source content
  enum class TonemapStrategy {
    // Use a tonemapper defined by libtonemap. This may be OEM-defined as of
    // Android 13, aka
    // undefined.
    // This is typically a global tonemapper, designed to match what is on
    // screen.
    Libtonemap,
    // Use a local tonemapper. Because local tonemapping uses large intermediate
    // allocations,
    // this
    // method is primarily recommended for infrequent rendering that does not
    // need to exactly
    // match
...
static inline bool operator==(const DisplaySettings &lhs,
                              const DisplaySettings &rhs) {
  return lhs.namePlusId == rhs.namePlusId &&
         lhs.physicalDisplay == rhs.physicalDisplay && lhs.clip == rhs.clip &&
...
static inline void PrintTo(const DisplaySettings &settings,
                           ::std::ostream *os) {
  *os << "DisplaySettings {";
  ...
  *os << "\n    .deviceHandlesColorTransform = "
      << settings.deviceHandlesColorTransform;
  *os << "\n    .orientation = " << orientation_to_string(settings.orientation);
  *os << "\n    .targetLuminanceNits = " << settings.targetLuminanceNits;
  *os << "\n    .dimmingStage = "
      << aidl::android::hardware::graphics::composer3::toString(
             settings.dimmingStage)
             .c_str();
  *os << "\n    .renderIntent = "
      << aidl::android::hardware::graphics::composer3::toString(
             settings.renderIntent)
             .c_str();
  *os << "\n}";
}
