...
struct Geometry {
  ...
  // Having corner radius will force GPU composition on the layer and its
  // children, drawing it with a special shader. The shader will receive the
  // radius and the crop rectangle as input, modifying the opacity of the
  // destination texture, multiplying it by a number between 0 and 1. We query
  // Layer#getRoundedCornerState() to retrieve the radius as well as the rounded
  // crop rectangle to figure out how to apply the radius for this layer. The
  // crop rectangle will be in local layer coordinate space, so we have to take
  // the layer transform into account when walking up the tree.
...
static inline bool operator==(const Geometry &lhs, const Geometry &rhs) {
  return lhs.boundaries == rhs.boundaries &&
         lhs.positionTransform == rhs.positionTransform &&
...
static inline bool operator==(const LayerSettings &lhs,
                              const LayerSettings &rhs) {
  ...
  return lhs.geometry == rhs.geometry && lhs.source == rhs.source &&
         lhs.alpha == rhs.alpha && lhs.sourceDataspace == rhs.sourceDataspace &&
         lhs.colorTransform == rhs.colorTransform &&
         lhs.disableBlending == rhs.disableBlending &&
         lhs.skipContentDraw == rhs.skipContentDraw &&
...
static inline void PrintTo(const Buffer &settings, ::std::ostream *os) {
  ...
  *os << "\n    .buffer = " << settings.buffer.get() << " "
      << (settings.buffer.get()
              ? decodePixelFormat(settings.buffer->getPixelFormat()).c_str()
              : "");
...
static inline void PrintTo(const EdgeExtensionEffect &effect,
                           ::std::ostream *os) {
  *os << effect;
}
