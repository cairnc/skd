 ...
 *
 * Clients of RenderEngine are required to wrap their GraphicBuffer objects as
 * an ExternalTexture, which is then mapped into GPU resources required by
 * RenderEngine. When a client no longer needs to use the GraphicBuffer as input
 * into RenderEngine::drawLayers, then the client should delete their
 * ExternalTexture so that resources may be freed.
 */
...
public:
  ...
  Rect getBounds() const {
    return {0, 0, static_cast<int32_t>(getWidth()),
