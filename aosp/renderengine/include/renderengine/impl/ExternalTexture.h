...
public:
  ...
  enum Usage : uint32_t {
    // When a buffer is not READABLE but is WRITEABLE, then GLESRenderEngine
    // will use that as a
    // hint to load the buffer into a separate cache
    READABLE = 1 << 0,

    // The buffer needs to be mapped as a 2D texture if set, otherwise must be
    // mapped as an
    // external texture
    WRITEABLE = 1 << 1,
  };

  // Creates an ExternalTexture for the provided buffer and RenderEngine
  // instance, with the given usage hint of type Usage.
  ExternalTexture(const sp<GraphicBuffer> &buffer,
                  android::renderengine::RenderEngine &renderEngine,
                  uint32_t usage);
  ~ExternalTexture();
  ...
  PixelFormat getPixelFormat() const override {
    return getBuffer()->getPixelFormat();
  }
