 ...
 * that keeps GPU resources alive until the last SkImage or SkSurface object
 * using them is destroyed.
 */
...
public:
  ...
  // ensuring that access to this class is thread safe.  Clients also control
  // when the resources are reclaimed by setting the manager into deferred mode.
  ...
  // Local reference that supports RAII-style management of an
  // AutoBackendTexture AutoBackendTexture by itself can't be managed in a
  // similar fashion because of shared ownership with Skia objects, so we wrap
  // it here instead.
  class LocalRef {
  public:
    LocalRef(std::unique_ptr<SkiaBackendTexture> backendTexture,
    ...
    SkColorType colorType() const {
      return mTexture->mBackendTexture->internalColorType();
    }
...
private:
  ...
  // Creates an AutoBackendTexture to manage the lifecycle of a given
  // SkiaBackendTexture, which is in turn backed by an underlying
