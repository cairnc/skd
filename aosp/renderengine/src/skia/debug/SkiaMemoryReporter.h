...
// Mapping of resource substrings (1st element) that if found within a trace
// "dumpName" should be mapped to the category name (2nd element). All char*
// used in a resourcePair are expected to have a lifetime longer than the
// SkiaMemoryReporter in which they are used.
typedef std::pair<const char *, const char *> ResourcePair;

/*
 * Utility class for logging the CPU/GPU usage of Skia caches in a format that
 * is specific to RenderEngine.  HWUI has a similar logging class, but the data
 * collected and the way it is formatted and reported on are intended to be
 * unique to each use case.
 */
class SkiaMemoryReporter : public SkTraceMemoryDump {
public:
  /**
   * Creates the reporter class that can be populated by various Skia entry
   * points, like SkGraphics and GrContext, as well as format and log the
   * results.
   * @param resourceMap An array of values that maps a Skia dumpName into a user
   * defined category. The first vector entry that matches the dumpName is used
   * for the mapping.
   * @param itemize if true when logging the categories the individual elements
   * will be printed directly after the category details are printed. Otherwise,
   * only the category totals will be printed.
   */
  SkiaMemoryReporter(const std::vector<ResourcePair> &resourceMap,
                     bool itemize);
  ...
  void dumpNumericValue(const char *dumpName, const char *valueName,
                        const char *units, uint64_t value) override;

  void dumpStringValue(const char *dumpName, const char *valueName,
...
private:
  ...
  // variable that stores the final format of the data after the individual
