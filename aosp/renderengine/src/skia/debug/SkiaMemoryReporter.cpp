...
SkiaMemoryReporter::SkiaMemoryReporter(
    const std::vector<ResourcePair> &resourceMap, bool itemize)
    : mResourceMap(resourceMap), mItemize(itemize), mTotalSize("bytes", 0),
...
void SkiaMemoryReporter::processCurrentElement() {
  ...
  const char *resourceName =
      mCurrentElement.empty() ? nullptr : mapName(mCurrentElement.c_str());
  ...
  // Only count elements that contain "size"; other values just provide
  // metadata.
  ...
  // insert a copy of the element and all of its keys. We must make a copy here
  // instead of std::move() as we will continue to use these values later in the
  // function and again when we move on to process the next element.
  ...
  if (result != results.end()) {
    ...
    if (totalResult != resourceValues.end()) {
      ALOGE_IF(sizeResult->second.units != totalResult->second.units,
               "resource units do not match so the sum of resource type (%s) "
               "will be invalid",
               resourceName);
      totalResult->second.value += sizeResult->second.value;
      totalResult->second.count++;
    } else {
      ALOGE("an entry (%s) should not exist in the results without a size",
            resourceName);
    }
  } else {
...
void SkiaMemoryReporter::dumpNumericValue(const char *dumpName,
...
void SkiaMemoryReporter::dumpWrappedState(const char *dumpName,
                                          bool isWrappedObject) {
  if (mCurrentElement != dumpName) {
    processCurrentElement();
    mCurrentElement = dumpName;
  }
    ...
    if (categoryItem != resultsMap.end()) {
      ...
      if (result != categoryItem->second.end()) {
        ...
        StringAppendF(&log, "  %s: %.2f %s (%d %s)\n",
                      categoryItem->first.c_str(), traceValue.value,
                      traceValue.units.c_str(), traceValue.count, entry);
      }
      if (mItemize) {
        for (const auto &individualItem : resultsMap) {
          // if the individual item matches the category then print all its
          // details or in the case of wrapped resources just print the wrapped
          // size
          const char *categoryMatch = mapName(individualItem.first.c_str());
          if (categoryMatch &&
              strcmp(categoryMatch, resourceCategory.second) == 0) {
            auto result = individualItem.second.find("size");
            TraceValue size = convertUnits(result->second);
            StringAppendF(&log, "    %s: size[%.2f %s]",
                          individualItem.first.c_str(), size.value,
                          size.units.c_str());
            if (!wrappedResources) {
...
void SkiaMemoryReporter::logTotals(std::string &log) {
  ...
  StringAppendF(&log, " %.0f bytes, %.2f %s (%.2f %s is purgeable)\n",
                mTotalSize.value, total.value, total.units.c_str(),
                purgeable.value, purgeable.units.c_str());
}

SkiaMemoryReporter::TraceValue
SkiaMemoryReporter::convertUnits(const TraceValue &value) {
