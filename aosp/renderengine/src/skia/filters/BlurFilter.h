...
public:
  ...
  virtual sk_sp<SkImage> generate(SkiaGpuContext *context,
   ...
   * @param effectRegion the RoundRect in canvas coordinates that determines the
   * blur coverage
   * @param blurRadius radius of the blur used to determine the intensity of the
   * crossfade effect
   * @param blurAlpha alpha value applied to the effectRegion when the blur is
   * drawn
   * @param blurRect bounds of the blurredImage translated into canvas
   * coordinates
   * @param blurredImage down-sampled blurred content that was produced by the
   * generate() method
   * @param input original unblurred input that is used to crossfade with the
   * blurredImage
   */
...
private:
  // To avoid downscaling artifacts, we interpolate the blurred fbo with the
  // full composited image, up to this radius.
