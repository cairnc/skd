...
// Draws the given runtime shader on a GPU surface and returns the result as an
// SkImage.
sk_sp<SkImage> makeImage(SkSurface *surface,
                         const SkRuntimeShaderBuilder &builder) {
...
MouriMap::MouriMap()
    : mCrosstalkAndChunk16x16(makeEffect(kCrosstalkAndChunk16x16)),
      mChunk8x8(makeEffect(kChunk8x8)), mBlur(makeEffect(kBlur)),
      mTonemap(makeEffect(kTonemap)) {}

sk_sp<SkShader> MouriMap::mouriMap(SkiaGpuContext *context,
                                   sk_sp<SkShader> input, float hdrSdrRatio,
...
sk_sp<SkImage> MouriMap::downchunk(SkiaGpuContext *context,
  ...
  // TODO: fp16 might be overkill. Most practical surfaces use 8-bit RGB for HDR
  // UI and 10-bit YUV for HDR video. These downsample operations compute
  // log2(max(linear RGB, 1.0)). So we don't care about LDR precision since they
  // all resolve to LDR-max. For appropriately mastered HDR content that follows
  // BT. 2408, 25% of the bit range for HLG and 42% of the bit range for PQ are
  // reserved for HDR. This means that we can fit the entire HDR range for
  // 10-bit HLG inside of 8 bits. We can also fit about half of the range for
  // PQ, but most content does not fill the entire 10k nit range for PQ.
  // Furthermore, we blur all of this later on anyways, so we might not need to
  // be so precise. So, it's possible that we could use A8 or R8 instead. If we
  // want to be really conservative we can try to use R16 or even RGBA1010102 to
  // fake an R10 surface, which would cut write bandwidth significantly.
  ...
  LOG_ALWAYS_FATAL_IF(!firstDownsampledSurface, "%s: Failed to create surface!",
                      __func__);
  ...
  chunk8x8Builder.child("bitmap") = firstDownsampledImage->makeRawShader(
      SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions());
  static constexpr auto kSecondDownscaleAmount = 8;
  sk_sp<SkSurface> secondDownsampledSurface =
      context->createRenderTarget(firstDownsampledImage->imageInfo().makeWH(
          std::max(1, firstDownsampledImage->width() / kSecondDownscaleAmount),
          std::max(1,
                   firstDownsampledImage->height() / kSecondDownscaleAmount)));
  LOG_ALWAYS_FATAL_IF(!secondDownsampledSurface,
                      "%s: Failed to create surface!", __func__);
...
sk_sp<SkImage> MouriMap::blur(SkiaGpuContext *context, SkImage *input) const {
  ...
  blurBuilder.child("bitmap") = input->makeRawShader(
      SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions());
  sk_sp<SkSurface> blurSurface =
      context->createRenderTarget(input->imageInfo());
...
sk_sp<SkShader> MouriMap::tonemap(sk_sp<SkShader> input, SkImage *localLux,
  ...
  tonemapBuilder.child("lux") = localLux->makeRawShader(
