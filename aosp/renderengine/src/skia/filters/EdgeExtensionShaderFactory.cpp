...
EdgeExtensionShaderFactory::EdgeExtensionShaderFactory() {
  ...
  mResult = std::make_unique<SkRuntimeEffect::Result>(
      SkRuntimeEffect::MakeForShader(edgeShader));
...
sk_sp<SkShader>
EdgeExtensionShaderFactory::createSkShader(const sk_sp<SkShader> &inputShader,
  ...
  if (imageBounds.isEmpty()) {
    builder.uniform("uImgSize") =
        SkPoint{layer.geometry.boundaries.getWidth(),
                layer.geometry.boundaries.getHeight()};
  } else {
    builder.uniform("uImgSize") =
        SkPoint{imageBounds.width(), imageBounds.height()};
  }
