...
// layerviewer: Vulkan backend paths compiled out; provide enough of the
// VkImageInfo shape for diagnostic logging that mentions it.
struct GrVkImageInfo {
  int fFormat = 0;
  unsigned fSampleCount = 0;
  unsigned fLevelCount = 0;
};
namespace GrBackendTextures {
inline int GetVkImageInfo(const GrBackendTexture &, GrVkImageInfo *) {
  return 0;
}
} // namespace GrBackendTextures
...
GaneshBackendTexture::GaneshBackendTexture(sk_sp<GrDirectContext> grContext,
  ...
  const bool createProtectedImage =
      0 != (desc.usage & AHARDWAREBUFFER_USAGE_PROTECTED_CONTENT);

  GrBackendFormat backendFormat;
  const GrBackendApi graphicsApi = grContext->backend();
  if (graphicsApi == GrBackendApi::kOpenGL) {
    backendFormat = GrAHardwareBufferUtils::GetGLBackendFormat(
        grContext.get(), desc.format, false);
    mBackendTexture = GrAHardwareBufferUtils::MakeGLBackendTexture(
        grContext.get(), buffer, desc.width, desc.height, &mDeleteProc,
        &mUpdateProc, &mImageCtx, createProtectedImage, backendFormat,
        isOutputBuffer);
  } else if (graphicsApi == GrBackendApi::kVulkan) {
    backendFormat = GrAHardwareBufferUtils::GetVulkanBackendFormat(
        grContext.get(), buffer, desc.format, false);
    mBackendTexture = GrAHardwareBufferUtils::MakeVulkanBackendTexture(
        grContext.get(), buffer, desc.width, desc.height, &mDeleteProc,
        &mUpdateProc, &mImageCtx, createProtectedImage, backendFormat,
        isOutputBuffer);
  } else {
    LOG_ALWAYS_FATAL("Unexpected graphics API %u",
                     static_cast<unsigned>(graphicsApi));
  }

  if (!mBackendTexture.isValid() || !desc.width || !desc.height) {
    LOG_ALWAYS_FATAL(
        "Failed to create a valid texture. [%p]:[%d,%d] isProtected:%d "
...
sk_sp<SkImage>
GaneshBackendTexture::makeImage(SkAlphaType alphaType, ui::Dataspace dataspace,
  ...
  sk_sp<SkImage> image = SkImages::BorrowTextureFrom(
      mGrContext.get(), mBackendTexture, kTopLeft_GrSurfaceOrigin, colorType,
      alphaType, toSkColorSpace(dataspace), releaseImageProc, releaseContext);
  if (!image) {
...
sk_sp<SkSurface>
GaneshBackendTexture::makeSurface(ui::Dataspace dataspace,
                                  TextureReleaseProc releaseSurfaceProc,
                                  ReleaseContext releaseContext) {
  const SkColorType colorType = internalColorType();
  sk_sp<SkSurface> surface = SkSurfaces::WrapBackendTexture(
      mGrContext.get(), mBackendTexture, kTopLeft_GrSurfaceOrigin, 0, colorType,
      toSkColorSpace(dataspace), nullptr, releaseSurfaceProc, releaseContext);
  if (!surface) {
...
void GaneshBackendTexture::logFatalTexture(const char *msg,
  ...
  case GrBackendApi::kOpenGL: {
    ...
    LOG_ALWAYS_FATAL(
        "%s isTextureValid:%d dataspace:%d"
