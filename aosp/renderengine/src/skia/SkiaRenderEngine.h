...
public:
  static std::unique_ptr<SkiaRenderEngine>
...
protected:
  ...
  // cleaning up backend-specific state. SkiaGpuContexts are invalid after
  // invocation.
  void finishRenderingAndAbandonContexts();

  // Functions that a given backend (GLES, Vulkan) must implement
  using Contexts =
      std::pair<unique_ptr<SkiaGpuContext>, unique_ptr<SkiaGpuContext>>;
  ...
  virtual void waitFence(SkiaGpuContext *context,
                         base::borrowed_fd fenceFd) = 0;
...
private:
  ...
  getOrCreateBackendTexture(const sp<GraphicBuffer> &buffer,
                            bool isOutputBuffer) REQUIRES(mRenderingMutex);
  ...
  // If requiresLinearEffect is true or the layer has a stretchEffect a new
  // shader is returned. Otherwise it returns the input shader.
  ...
  sk_sp<SkShader>
  createRuntimeEffectShader(const RuntimeEffectShaderParameters &);
  ...
  // Number of external holders of ExternalTexture references, per GraphicBuffer
  // ID.
  std::unordered_map<GraphicBufferId, int32_t>
      mGraphicBufferExternalRefs GUARDED_BY(mRenderingMutex);
  std::unordered_map<GraphicBufferId,
                     std::shared_ptr<AutoBackendTexture::LocalRef>>
      mTextureCache GUARDED_BY(mRenderingMutex);
  std::unordered_map<shaders::LinearEffect, sk_sp<SkRuntimeEffect>,
                     shaders::LinearEffectHasher>
      mRuntimeEffects;
  AutoBackendTexture::CleanupManager
      mTextureCleanupMgr GUARDED_BY(mRenderingMutex);
  ...
  // rendering that is potentially modified by multiple threads is guaranteed
  // thread-safe.
