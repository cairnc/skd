 ...
 * @param hdrSdrRatio default is 1.f, render engine side doesn't take care of
 * it.
 * @return HdrRenderType
 */
inline HdrRenderType
getHdrRenderType(ui::Dataspace dataspace,
  ...
  static const auto BT2020_LINEAR_EXT = static_cast<ui::Dataspace>(
      HAL_DATASPACE_STANDARD_BT2020 | HAL_DATASPACE_TRANSFER_LINEAR |
      HAL_DATASPACE_RANGE_EXTENDED);

  if ((dataspace == BT2020_LINEAR_EXT ||
       dataspace == ui::Dataspace::V0_SCRGB) &&
      pixelFormat.has_value() &&
      pixelFormat.value() == ui::PixelFormat::RGBA_FP16) {
    return HdrRenderType::GENERIC_HDR;
  }

  // Extended range layer with an hdr/sdr ratio of > 1.01f can "self-promote" to
  // HDR.
