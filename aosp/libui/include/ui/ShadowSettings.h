...
/*
 * Contains the configuration for the shadows drawn by single layer. Shadow
...
struct ShadowSettings {
  ...
  // Color to the spot shadow. The alpha is premultiplied. The position of the
  // spot shadow depends on the light position.
  vec4 spotColor = vec4();
  ...
  // If true fill in the casting layer is translucent and the shadow needs to
  // fill the bounds. Otherwise the shadow will only be drawn around the edges
  // of the casting layer.
  bool casterIsTranslucent = false;
};

static inline bool operator==(const ShadowSettings &lhs,
                              const ShadowSettings &rhs) {
