...
// ID of a physical or a virtual display. This class acts as a type safe wrapper
// around uint64_t. The encoding of the ID is type-specific for bits 0 to 61.
struct DisplayId {
...
inline bool operator!=(DisplayId lhs, DisplayId rhs) { return !(lhs == rhs); }
...
// DisplayId of a physical display, such as the internal display or externally
// connected display.
struct PhysicalDisplayId : DisplayId {
  ...
  static constexpr PhysicalDisplayId
  fromEdid(uint8_t port, uint16_t manufacturerId, uint32_t modelHash) {
  ...
  constexpr uint16_t getManufacturerId() const {
    return static_cast<uint16_t>(value >> 40);
  }
  constexpr uint8_t getPort() const { return static_cast<uint8_t>(value); }

private:
  constexpr PhysicalDisplayId(uint64_t flags, uint8_t port,
                              uint16_t manufacturerId, uint32_t modelHash)
...
protected:
  explicit constexpr VirtualDisplayId(uint64_t value)
      : DisplayId(FLAG_VIRTUAL | value) {}
  explicit constexpr VirtualDisplayId(DisplayId other) : DisplayId(other) {}
};

struct HalVirtualDisplayId : VirtualDisplayId {
  explicit constexpr HalVirtualDisplayId(BaseId baseId)
      : VirtualDisplayId(baseId) {}
...
private:
  explicit constexpr HalVirtualDisplayId(DisplayId other)
      : VirtualDisplayId(other) {}
};

struct GpuVirtualDisplayId : VirtualDisplayId {
  explicit constexpr GpuVirtualDisplayId(BaseId baseId)
      : VirtualDisplayId(FLAG_GPU | baseId) {}

  static constexpr std::optional<GpuVirtualDisplayId>
  fromUniqueId(const std::string &uniqueId) {
...
private:
  ...
  explicit constexpr GpuVirtualDisplayId(DisplayId other)
      : VirtualDisplayId(other) {}
};

// HalDisplayId is the ID of a display which is managed by HWC.
// PhysicalDisplayId and HalVirtualDisplayId are implicitly convertible to
// HalDisplayId.
...
template <> struct hash<android::DisplayId> {
...
template <> struct hash<android::HalDisplayId> : hash<android::DisplayId> {};
