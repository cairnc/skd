...
public:
  ...
  ColorSpace(const std::string &name, const std::array<float2, 3> &primaries,
             const float2 &whitePoint, transfer_function OETF = linearResponse,
             transfer_function EOTF = linearResponse,
             clamping_function clamper = saturate<float>) noexcept;
  ...
  constexpr const mat3 &getRGBtoXYZ() const noexcept { return mRGBtoXYZ; }

  constexpr const mat3 &getXYZtoRGB() const noexcept { return mXYZtoRGB; }

  constexpr const transfer_function &getOETF() const noexcept { return mOETF; }

  constexpr const transfer_function &getEOTF() const noexcept { return mEOTF; }
  ...
  constexpr const float2 &getWhitePoint() const noexcept { return mWhitePoint; }
...
private:
  static constexpr mat3 computeXYZMatrix(const std::array<float2, 3> &primaries,
                                         const float2 &whitePoint);

  static constexpr float linearResponse(float v) { return v; }
